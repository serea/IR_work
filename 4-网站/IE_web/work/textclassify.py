...
def bayesfastinitial(conprobname = 'work/conprob.txt'):
...
def d_bayestestonefile(content):
    # f = open(filename)
    # content = f.read().strip()
    conprob = bayesfastinitial()
    catedict = bayes(content, conprob)
...
def kNNinitial(tfidfname = 'work/tfidf.txt'):
    tfidf = []
    tdnum = []
    category = []
...
def d_kNNtestonefile(content, N=25):
    # f = open(filename)
    # content = f.read().strip()
    tfidf, category = kNNinitial()
    catedict = kNN(content, tfidf, category, N)
